<template>
	<view style="background-color: #EFEFF4;">
		...
		<view class="item">
			...
			<picker class="itemValue" mode="date" @change="bindJDDateChange"><input class="input" placeholder="请选择接待日期" v-model="jddate" type="text" /></picker>
		</view>
		<view class="hourLine"></view>
		...
		<view class="item">
			<text class="itemNamearea" style="display: flex;justify-content: center;align-items: center;">随行人员*:</text>
			<textarea class="inputarea" :auto-height="true" placeholder="请输入随行人员" placeholder-style="lineHeight:150rpx" v-model="sxry" type="text"  />
		</view>
		<view class="hourLine"></view>
		
		<view class="item">
			<text class="itemNamearea" style="display: flex;justify-content: center;align-items: center;">来宿事由*:</text>
			<textarea class="inputarea" :auto-height="true" placeholder="请输入来宿事由" placeholder-style="lineHeight:150rpx" v-model="reason" type="text"  />
		</view>
		<view class="hourLine"></view>
		
		<view class="item">
			<text class="itemNamearea" style="display: flex;justify-content: center;align-items: center;">陪客人员*:</text>
			<textarea class="inputarea" :auto-height="true" placeholder="请输入陪客人员" placeholder-style="lineHeight:150rpx" v-model="pkry" type="text"/>
		</view>
		<view class="hourLine"></view>
...
export default {
	...
	methods: {
		...
		bindJDDateChange: function(e) {
			this.jddate = e.target.value;
		},
		setData: function(e) {
		...
		getFlowLine: function(e) {
			...
			resquestJSOB.sendGetRequestJson({
				...
				data: {
					defId: dataString.receptionDefId
				},
		...
		//上传数据
		UpData:function(e){
			if (!this.guest || !this.guestwork || !this.lbdepartment || !this.sxry || !this.reason
				|| !this.pkry || !this.eatAddress || !this.sleepAddress || !this.jdbz) {
				uni.showToast({
					title: '*号为必填内容',
					icon: 'none',
					duration: 800,
			...
			myData.append('jdbm', uni.getStorageSync('depName'));
			myData.append('tbrq', this.tddate);
			myData.append('jdrq', this.jddate);
			myData.append('jdsj', this.jddate);
			myData.append('lbdw', this.lbdepartment);
			myData.append('lssy', this.reason);
			myData.append('xm', this.guest);
			myData.append('zw', this.guestwork);
			myData.append('sxry', this.sxry);
			myData.append('pkry', this.pkry);
			myData.append('jcdd', this.eatAddress);
			myData.append('zsdd', this.sleepAddress);
			myData.append('jdbz', this.jdbz);
