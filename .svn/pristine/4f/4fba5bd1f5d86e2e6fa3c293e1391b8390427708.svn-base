    ...
    <view class="modal plane flex-row" :class="show && 'active'">
      ...
      <view class="plane-content">
        <view class="modal-box">
          <view class="modal-title hide-text-1" v-if="title">{{title}}</view>
          <view class="partation"></view>
          <view class="modal-content" v-if="content">{{content}}</view>
          <view class="flex-row">
            <button type="default" v-if="cancelText" size="mini" @click="cancel">{{cancelText}}</button>
            <button type="primary" v-if="confirmText" size="mini" @click="confirm">{{confirmText}}</button>
          </view>
        </view>
      </view>
    </view>
