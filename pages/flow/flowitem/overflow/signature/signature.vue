	...
	methods: {
		...
		getFlowLine: function(e) {
			...
			resquestJSOB.sendGetRequestJson({
				...
				data: {
					defId: dataString.signatureDefId
				},
		...
		//上传数据
		UpData:function(e){
			if (!this.prove	 || !this.jb || !this.Scontent) {
				uni.showToast({
					title: '*号为必填内容',
					icon: 'none',
					duration: 800,
			...
			myData.append('neirong', this.Scontent);
			myData.append('zm', this.prove);
			myData.append('jb', this.jb);
			myData.append('bmfzryj', '');
			myData.append('fgldyj', '');
			myData.append('cwzjyj', '');
