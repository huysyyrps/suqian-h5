<template>
	<view>
		<view style="margin: 20rpx 20rpx; background-color: #FFFFFF; padding: 20rpx 30rpx; border-radius: 20rpx;">
			<input @input="onVuleChange" style="font-size: 30rpx;"  placeholder="输入车牌号" />
		</view>
		<uni-list>
	...
	export default {
		...
		methods: {
			clickListItem(data){
				...
				prevPage.$vm.carNo = data.name
				prevPage.$vm.busCode = data.value
			...
			sendDataRequest() { //
				var that = this;
				try{
					var data = uni.getStorageSync("carNo")
					if(data != ""){
						that.listData = data
						that.allData = data
						return
				...
				resquestJSOB.sendGetRequestJson({
					...
					successCallBack: function(data) {
						...
						try{
							uni.setStorageSync("carNo", data.data)
						}catch(e){
							//TODO handle the exception
						}
				},
		  ...
		  onVuleChange(event) {
			...
			fuzzyQuery(keyWord){
				...
				for (var i = 0; i < originalData.length; i++) {
					...
					console.log(obj.name+keyWord)
					console.log(obj.name,keyWord,stringUtil.testString(obj.name,keyWord))
					if(stringUtil.testString(obj.name,keyWord)){
						newData.push(obj)
					}
				}
				return newData
